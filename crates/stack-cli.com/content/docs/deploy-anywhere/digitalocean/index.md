# Deploy Anywhere: DigitalOcean Droplet

This guide shows how to take a single DigitalOcean Droplet, install k3s, and run multiple Stack apps on the same VM.

## 1) Provision the droplet

- Create a new Droplet (Ubuntu 22.04 LTS).
- Choose a plan with at least 2 vCPU and 4 GB RAM.
- Add your SSH key and note the public IP.
- Allow inbound TCP on 22, 80, 443, and 6443 (lock 6443 to your IP if possible).
...
Each StackApp gets its own namespace, database, and auth service, all on the same Droplet.

## 5) Expose apps to the internet

- For quick exposure, use the [Ingress](../../ingress/) guide to add a tunnel per StackApp.
- For production ingress, point your DNS at the droplet and configure an ingress controller or load balancer.
