...
pub async fn deploy(
    ...
    let env = vec![
        ...
        json!({"name": "DB_AFTER_CONNECT_QUERY", "value": "SET search_path TO realtime"}),
        json!({
            "name": "API_JWT_SECRET",
            "valueFrom": {
                "secretKeyRef": {
    ...
    let init_container = deployment::InitContainer {
        ...
        command: Some(deployment::Command {
            ...
            args: vec![
                "psql -v ON_ERROR_STOP=1 -c 'CREATE SCHEMA IF NOT EXISTS realtime;' -c 'ALTER SCHEMA realtime OWNER TO \"db-owner\";' -c \"DO \\$\\$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN CREATE PUBLICATION supabase_realtime; END IF; END \\$\\$;\""
                    .to_string(),
            ],
        }),
    };
