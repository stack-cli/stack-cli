...
This repository mixes several deliverables that move at different speeds. To keep things tidy, we split the work into three "agents" that each own one slice of the Stack experience. Use this guide to decide where changes belong and how to validate them before opening a PR.

## Documentation Agent (`crates/stack-cli.com`)
- **Purpose**: Publish the public docs + marketing site that explains Stack concepts and walks users through the CLI flows.
- **Primary tasks**: Update markdown in `content/`, assets in `assets/`, and Dioxus components in `src/`. Keep code samples aligned with the default Stack template and live CLI flags.
...
- **Purpose**: Own the CLI/operator that installs the Stack platform (CNPG, Keycloak, ingress, StackApps) into Kubernetes clusters.
